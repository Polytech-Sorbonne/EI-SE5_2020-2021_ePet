...
// seuils internes à la fonction accelero
#define SEUILX 2.5 
#define SEUILY 2.5
...
#define TOUR_MINUTES_ACC 6000/15
...
void setup()
{
    ...
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();
    btStop(); //desactiver le bluetooth
    delay(100);
    ...
    debug("Range = "); debug_int(2 << mma.getRange());  
    debug("G");

    debug("SHTC3 test");
  if (! shtc3.begin()) {
    debug("Couldn't find SHTC3");
  }
  debug("Found SHTC3 sensor");
}
...
bool testMacAdresse(String id_add){
  ...
  for(int i = 0; i < sizeof(mac) / sizeof(mac[0]); i++){ 
      ...
      if(id_add.equalsIgnoreCase(id_const)){
        debug("Les id sont identiques");
        debug("Id adresse MAC : ");
        Serial.print(id_add);
        debug("Id adresse MAC Constructeur : ");
        Serial.print(id_const);
        return false; 
      }  
   }
   return true; 
...
//Permet de definir le mouvement          
void accelerometre(Adafruit_MMA8451 mma){
  ...
  bool significantX = false;
  sensors_event_t event; 
  mma.getEvent(&event);
  
  // Read the 'raw' data in 14-bit counts
  mma.read();
  debug("X:\t"); debug_int(mma.x); 
  debug("\tY:\t"); debug_int(mma.y); 
  debug("\tZ:\t"); debug_int(mma.z); 
  ...
  for (int minutes = 0; minutes < 15; minutes ++)
  {
    ...
    x_0 = event.acceleration.x;
    y_0 = event.acceleration.y;
    for (int i = 0; i < TOUR_MINUTES_ACC; i++)
    {
      sensors_event_t event; 
      mma.getEvent(&event);
      mma.read();
  ...
  x = countmintX; //X prend le nombre de fois ou X a dépassé le seuil en 15 minutes
  y = countmintY; //Y prend le nombre de fois ou X a dépassé le seuil en 15 minutes
  debug("fin de l'accelero");
}
...
int temperature(){
  ...
  debug("Temperature: "); debug_int(temp.temperature); debug(" degrees C");
...
void format_message(){
  ...
  switch(cas){
   ...
   case 2: // Température normal & mouvement présent & pas chgmt localisation
     ...
     convByteToCharAcc(mouv_tabX, x_trad);
     convByteToCharAcc(mouv_tabY, y_trad);
